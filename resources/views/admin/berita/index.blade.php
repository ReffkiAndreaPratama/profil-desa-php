...
<div class="flex items-center justify-between mb-6">
    <form method="GET" class="flex gap-2">
        <input
            ...
            class="px-4 py-2 border-2 border-[#212121] rounded-xl text-sm
                   outline-none focus:border-[#2E7D32]"/>
        <button type="submit"
                class="brutal-btn bg-[#2E7D32] text-white px-4 py-2 rounded-xl text-sm">
            Cari
        </button>
    </form>
                ...
                @forelse($berita as $b)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-400">
                            {{ method_exists($berita, 'currentPage') ? ($berita->currentPage() - 1) * $berita->perPage() + $loop->iteration : $loop->iteration }}
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <span class="bg-[#E8F5E9] text-[#2E7D32] px-2 py-1 rounded-full text-xs font-bold">
                                {{ $b->kategori }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.berita.edit', $b->id) }}"
                                   class="brutal-btn bg-[#1565C0] text-white px-3 py-1.5 rounded-lg text-xs">
                                    ✏️ Edit
                                </a>
                                <form id="del-berita-{{ $b->id }}"
                                ...
                                <button type="button"
                                        onclick="showDeleteModal('{{ route('admin.berita.destroy', $b->id) }}', 'Hapus Berita', '{{ addslashes($b->judul) }}')"
