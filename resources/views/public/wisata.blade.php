...
<div class="min-h-screen bg-[#FFFDF7] dark:bg-[#121212] pt-24">
    ...
    @include('layouts.partials.page-header', [
        'title'    => 'Wisata Desa Talang Marap',
        'subtitle' => 'Jelajahi keindahan alam dan budaya desa',
    ])
    ...
    <div class="container-custom py-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            @foreach($wisataStats as $s)
                <div class="brutal-card p-4 text-center">
                    <span class="text-3xl">{{ $s['icon'] }}</span>
                    <p class="font-black text-xl text-[#2E7D32] mt-1">{{ $s['value'] }}</p>
        ...
        <h2 class="text-2xl font-black mb-6">
            Destinasi <span class="text-gradient">Unggulan</span>
        </h2>
            ...
            @foreach($wisata as $w)
                <div class="brutal-card overflow-hidden">
                    ...
                    <div class="relative h-52">
                        <img src="{{ \App\Helpers\FotoHelper::url($w->foto, 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=600') }}"
                             alt="{{ $w->nama }}"
                        ...
                        <div class="absolute top-4 right-4 bg-white/90 rounded-xl px-3 py-1
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <span class="text-[#2E7D32] font-black text-sm bg-[#E8F5E9]
                                         px-2 py-1 rounded-lg border border-[#2E7D32]">
                                {{ $w->harga }}
                            </span>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            @foreach($w->fasilitas ?? [] as $f)
                                <span class="text-xs bg-[#E8F5E9] text-[#2E7D32]
                                             px-2 py-1 rounded-full border border-[#66BB6A]">
                                    ✓ {{ $f }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        @if($w->maps)
                            <a href="{{ $w->maps }}"
                               target="_blank"
                               class="brutal-btn bg-[#2E7D32] text-white px-4 py-2 rounded-xl
                                      font-bold text-sm inline-flex items-center gap-2">
                                🗺️ Buka di Maps
                            </a>
                        @endif
                    </div>
