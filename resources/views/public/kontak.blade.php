...
@section('title', 'Kontak - Portal Desa Talang Marap')
@section('content')
<div class="min-h-screen bg-[#FFFDF7] pt-24">
    <div class="gradient-green border-b-4 border-[#212121] py-10">
        <div class="container-custom">
            <h1 class="text-3xl font-black text-white mb-1">Hubungi Kami</h1>
            <p class="text-white/70">Kantor Desa Talang Marap siap melayani Anda</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                ...
                <div class="brutal-card p-5 flex items-center gap-4">
                    <div class="text-3xl w-12 h-12 flex items-center justify-center bg-[#E8F5E9] rounded-xl border-2 border-[#2E7D32] shrink-0">{{ $c['icon'] }}</div>
                    <div>
                ...
                @endforeach
                <a href="https://wa.me/{{ $desa['whatsapp'] ?? '6281234567890' }}" target="_blank"
                   class="brutal-btn bg-green-500 text-white w-full py-3 rounded-xl font-black text-center block mt-4">
                    💬 Chat via WhatsApp
                </a>
            </div>
                ...
                <form action="{{ route('kontak.submit') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required
                               class="w-full px-4 py-3 border-[3px] border-[#212121] rounded-xl outline-none focus:border-[#2E7D32] @error('email') border-red-500 @enderror"/>
                    ...
                    <button type="submit" class="w-full brutal-btn bg-[#2E7D32] text-white py-3 rounded-xl font-black">
                        Kirim Pesan →
                    </button>
                </form>
            </div>
