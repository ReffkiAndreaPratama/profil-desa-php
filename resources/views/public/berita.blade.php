...
@section('title', 'Berita Desa - Smart Village Talang Marap')
...
<div class="min-h-screen bg-[#FFFDF7] dark:bg-[#121212] pt-24">
    ...
    <div class="gradient-green border-b-4 border-[#212121] py-10">
        <div class="container-custom flex items-end justify-between">
            ...
            <span class="text-white/60 text-sm font-bold hidden md:block">
                {{ $berita->total() }} artikel
            </span>
        </div>
    </div>
    ...
    <div class="container-custom py-8">
        ...
        {{-- ── Filter & search ── --}}
        @php
        $kategoris = ['Semua', 'Pemerintahan', 'KKN', 'Lingkungan', 'Pertanian', 'Kesehatan', 'UMKM', 'Pendidikan'];
        @endphp
        ...
        <form method="GET" class="flex flex-col md:flex-row gap-4 mb-8">
            <input
                ...
                class="flex-1 px-4 py-3 border-[3px] border-[#212121] rounded-xl font-medium
                       outline-none focus:border-[#2E7D32] bg-white shadow-[3px_3px_0_#212121]"/>
            <div class="flex gap-2 flex-wrap">
                @foreach($kategoris as $k)
                    <button
                        ...
                        name="kategori"
                        value="{{ $k }}"
                        class="brutal-btn px-4 py-2 rounded-xl font-bold text-sm
                               {{ request('kategori', 'Semua') === $k
