...
<div class="min-h-screen bg-[#FFFDF7] dark:bg-[#121212] pt-24">
    ...
    @include('layouts.partials.page-header', [
        ...
        'subtitle' => 'Agenda desa dan jadwal kegiatan',
    ])
        ...
        <div class="space-y-4">
            @forelse($agenda as $ag)
                <div class="brutal-card p-5 flex items-center gap-4">
                    ...
                    <div class="bg-[#2E7D32] text-white w-16 h-16 rounded-xl flex flex-col
                    ...
                    <div class="flex-1">
                        <h3 class="font-black text-base dark:text-gray-200">{{ $ag->judul }}</h3>
                        <p class="text-gray-500 text-sm mt-1">
                            ⏰ {{ $ag->jam }} · 📍 {{ $ag->lokasi }}
                        </p>
                        @if($ag->deskripsi)
                            <p class="text-gray-400 text-xs mt-1">{{ $ag->deskripsi }}</p>
                        @endif
                    </div>

                    <span class="text-xs bg-[#E8F5E9] border border-[#2E7D32] text-[#2E7D32]
                                 px-3 py-1 rounded-full font-semibold shrink-0 hidden sm:inline">
                        {{ $ag->kategori }}
                    </span>
            ...
            @empty
                <div class="brutal-card p-12 text-center text-gray-400">
                    ...
                    <p class="font-bold">Belum ada agenda</p>
                </div>
            @endforelse
        </div>
