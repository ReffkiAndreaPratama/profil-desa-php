    ...
    <div class="bg-white border-4 border-[#212121] rounded-2xl shadow-[4px_4px_0_#212121] overflow-hidden">
        ...
        <div class="bg-[#2E7D32] p-4">
            <h3 class="font-black text-white">
                {{ $item ? '✏️ Edit' : '➕ Tambah' }} {{ $title }}
            </h3>
        </div>

        <form action="{{ $item ? route($updateRoute, $item->id) : route($storeRoute) }}"
              method="POST"
              class="p-6 space-y-5">
            @csrf
            @if($item)
                ...
                <div>
                    ...
                    @if(($field['type'] ?? 'text') === 'textarea')
                        <textarea
                            ...
                            class="w-full px-4 py-3 border-[3px] border-[#212121] rounded-xl
                                   outline-none focus:border-[#2E7D32] resize-none">{{ old($field['name'], $item->{$field['name']} ?? '') }}</textarea>
                    ...
                    @elseif(($field['type'] ?? 'text') === 'select')
                        <select
                            ...
                            @foreach($field['options'] as $opt)
                                <option
                                    value="{{ $opt }}"
                                    {{ old($field['name'], $item->{$field['name']} ?? '') === $opt ? 'selected' : '' }}>
                                    {{ $opt }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @elseif(($field['type'] ?? 'text') === 'checkbox')
                        <div class="flex items-center gap-2">
                            ...
                            <input
                                ...
                                name="{{ $field['name'] }}"
                                value="1"
                                {{ old($field['name'], $item->{$field['name']} ?? true) ? 'checked' : '' }}
                                class="w-4 h-4"/>
                    ...
                    @else
                        <input
                            ...
                            name="{{ $field['name'] }}"
                            value="{{ old($field['name'], is_array($item->{$field['name']} ?? null)
                                ? implode(', ', $item->{$field['name']})
