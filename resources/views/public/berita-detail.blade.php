...
@section('title', $berita->judul . ' - Smart Village Talang Marap')
...
<div class="min-h-screen bg-[#FFFDF7] dark:bg-[#121212] pt-24">
    <div class="container-custom py-8 max-w-4xl">
        ...
        {{-- ── Back link ── --}}
        <a href="{{ route('berita') }}"
           class="inline-flex items-center gap-2 font-bold text-[#2E7D32]
                  hover:gap-3 transition-all mb-6 text-sm">
            ← Kembali ke Berita
        </a>
        ...
        <div class="brutal-card overflow-hidden">
            ...
            @if($berita->foto)
                <img src="{{ \App\Helpers\FotoHelper::url($berita->foto) }}"
                     alt="{{ $berita->judul }}"
            ...
            <div class="p-6 md:p-8">
                ...
                <span class="inline-block bg-[#E8F5E9] text-[#2E7D32] text-xs font-bold
                             px-3 py-1 rounded-full border border-[#2E7D32] mb-4">
                    {{ $berita->kategori }}
                </span>

                <h1 class="text-2xl md:text-3xl font-black text-[#212121] mb-4">
                    {{ $berita->judul }}
                </h1>
                ...
                @if($berita->ringkasan)
                    <p class="text-gray-700 leading-relaxed text-base mb-4 font-medium italic
                              border-l-4 border-[#2E7D32] pl-4">
                        {{ $berita->ringkasan }}
                    </p>
                @endif
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 flex flex-wrap items-center gap-3">
                    ...
                    <a href="https://wa.me/?text={{ $shareTitle }}%20{{ $shareUrl }}"
                       target="_blank"
                       class="brutal-btn text-xs px-3 py-1.5 rounded-lg bg-green-100 text-green-700
                              hover:bg-green-600 hover:text-white transition-colors">
                        WhatsApp
                    </a>
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ $shareUrl }}"
                       target="_blank"
                       class="brutal-btn text-xs px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700
                              hover:bg-blue-600 hover:text-white transition-colors">
                        Facebook
                    </a>
                    <a href="https://twitter.com/intent/tweet?text={{ $shareTitle }}&url={{ $shareUrl }}"
                       target="_blank"
                       class="brutal-btn text-xs px-3 py-1.5 rounded-lg bg-sky-100 text-sky-700
        ...
        @if($related->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @foreach($related as $r)
                    <a href="{{ route('berita.detail', $r->id) }}"
                       class="brutal-card overflow-hidden group">
                        @if($r->foto)
                            <img src="{{ \App\Helpers\FotoHelper::url($r->foto) }}"
                                 alt="{{ $r->judul }}"
